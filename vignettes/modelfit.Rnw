...
Saint-Andrieux et al. They are not exactly the same, as these results
are calculated from Monte Carlo simulations (i.e. random draws from
the posterior distribution, which can vary slightly between two
different executions).\\

Thus, the best model has slightly more than one chance out of two to
include the forest fragmentation and the hunting bag (as noted by
Saint-Andrieux et al.).
